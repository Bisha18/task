...
// claim history for specific user
export const claimHistoryForSpecificUser = async(req,res)=>{
  ...
  try {
    const history = await claimHistoryForSpecificUser.find({userId:userId}).populate('userId', 'username').sort({ timestamp: -1 });
    res.json(history);
  } catch (error) {
